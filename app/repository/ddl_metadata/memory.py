...
from app.repository.ddl_metadata.schema import llm_memory, llm_memory_relation
...
def _decode_cursor(cursor: str) -> tuple[datetime, int]:
    ...
    try:
        ...
        updated_at, identifier = json.loads(
...
class MemoryRepository:
    ...
    async def related_uids(
        ...
        result: dict[int, set[str]] = {
            memory_id: set() for memory_id in memory_ids
        }
    ...
    async def patch(
        ...
        *,
        ...
        if values:
            await self._session.execute(
                update(llm_memory)
                .where(llm_memory.c.id == memory.id)
                .values(**values)
            )
