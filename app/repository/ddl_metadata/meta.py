...
class MetaRepository:
    ...
    async def synchronize(
        ...
        semantic_columns = {
            column.column_id: column for column in metadata.columns
        }
        ...
        if stale_column_ids:
            await self._session.execute(
                delete(column_info).where(
                    column_info.c.id.in_(stale_column_ids)
                )
            )
    ...
    async def existing_object_ids(
        ...
        if metric_ids:
            found.update(
                (
                    await self._session.scalars(
                        select(metric_info.c.id).where(
                            metric_info.c.id.in_(metric_ids)
                        )
                    )
                ).all()
            )
