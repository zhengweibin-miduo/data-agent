...
from app.client.llm_client_manager import LlmClientManager
...
from app.service.ddl_metadata.llm import LlmMetadataModel
...
async def run_ddl_job(
    ...
    try:
        ...
        if isinstance(error, _RETRYABLE) and latest.attempt < 3:
            ...
            raise Retry(
                defer=(2 ** latest.attempt) + random.uniform(0, 1)
            ) from error
        await jobs.mark_terminal(
            job_id,
            revision,
...
async def cleanup_checkpoints(ctx: dict[Any, Any]) -> None:
    ...
    if not job_ids:
        return
    checkpointer = CheckpointClientManager.get_client()
...
async def startup(ctx: dict[Any, Any]) -> None:
    ...
    ctx["jobs"] = JobStore(redis)
    ctx["graph"] = build_ddl_metadata_graph(
        DdlGraphDependencies(
            model=LlmMetadataModel(),
