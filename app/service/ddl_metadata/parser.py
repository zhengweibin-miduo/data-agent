...
from app.model.ddl_metadata import PhysicalColumn, PhysicalSchema, PhysicalTable
from app.service.ddl_metadata.errors import DdlMetadataError
from app.service.ddl_metadata.identifiers import column_id, table_id
...
def _constraint_column_names(
    ...
    for item in schema.expressions:
        expressions = (
            item.expressions if isinstance(item, exp.Constraint) else [item]
        )
...
def parse_ddl(
    source: str,
    ddl: str,
    limits: ApiConfig = app_config.api,
) -> PhysicalSchema:
