...
from app.service.ddl_metadata.job_store import question_set_id
from app.service.ddl_metadata.memory_context import LoadedMemoryContext
from app.service.ddl_metadata.memory import build_accepted_memories
from app.service.ddl_metadata.llm import MetadataModel
from app.service.ddl_metadata.validator import (
...
def build_ddl_metadata_graph(
    ...
    async def load_memory_node(state: DdlGraphState) -> DdlGraphState:
        ...
        update: DdlGraphState = {
            "memory_capsule": [
                ...
                for content in context.semantic_capsule
            ],
            "reused_memory": [
                content.model_dump(mode="json")
                for content in context.reused_memory
            ],
            "metric_questions": [
    ...
    async def validate_metadata_node(state: DdlGraphState) -> DdlGraphState:
        schema = PhysicalSchema.model_validate(state.get("physical_schema"))
        metadata = SemanticMetadata.model_validate(
            state.get("semantic_metadata")
        )
    ...
    async def plan_questions_node(state: DdlGraphState) -> DdlGraphState:
        ...
        merged.update(
            {question.question_id: question for question in output.questions}
        )
        return {
            "metric_questions": [
                question.model_dump(mode="json")
                for question in merged.values()
            ],
            "current_questions": [
                question.model_dump(mode="json")
    ...
    async def await_answers_node(state: DdlGraphState) -> DdlGraphState:
        ...
        resumed = interrupt(
            {
                "questions": [
                    question.model_dump(mode="json") for question in current
                ],
        ...
        new_answers = [
            MetricAnswer.model_validate(answer) for answer in resumed
        ]
        existing = {
        ...
        existing.update(
            {answer.question_id: answer for answer in new_answers}
        )
        return {
            "metric_answers": [
    ...
    async def generate_metrics_node(state: DdlGraphState) -> DdlGraphState:
        ...
        return {
            "metrics": [
                metric.model_dump(mode="json") for metric in output.metrics
            ],
            "validation_errors": [],
            "route": "validate",
        }
    ...
    async def validate_metrics_node(state: DdlGraphState) -> DdlGraphState:
        ...
        metrics = [
            MetricMetadata.model_validate(metric)
        ...
        if not issues:
            return {
                "metrics": [
                    metric.model_dump(mode="json") for metric in finalized
                ],
                "validation_errors": [],
                "route": "valid",
            }
