...
from app.repository.ddl_metadata.memory import MemoryRepository, StoredMemory
...
from app.service.ddl_metadata.validator import validate_metadata
from app.service.ddl_metadata.validator import finalize_and_validate_metrics
...
def _choose_memory(
    ...
    if not choices:
        return None
    canonical = {
        memory.content.model_dump_json(exclude_none=True)
    ...
    async def load(
        ...
        for scope_key, memories in grouped.items():
            ...
            if content.table is not None and content.table.table_id == scope_key:
                ...
                content.column is not None
                and content.column.column_id == scope_key
            ):
        ...
        referenced_uids = {
            uid
            for uids in metric_reference_uids.values()
            for uid in uids
        }
        questions = [
        ...
        if metric_issues:
            ...
            finalized = []
        reused_metrics = {
            metric.id for metric in finalized
        }
