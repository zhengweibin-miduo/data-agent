...
已有 PR head 在任务执行期间发生变化时，按第 6.6 节的远端干预判定处理；
该节只允许吸收可证明安全的线性更新，不构成任意改写共享历史的许可。
...
- 提交信息必须使用中文准确描述实际改动；采用 Conventional Commits 时可保留 `feat:`、`fix:` 等类型前缀，但主题和正文必须使用中文。
...
同一 PR head 可能被其他 Codex 任务或协作者更新。远端 head 发生变化本身
不是停止条件；必须在已有 commit / push 授权范围内，根据 fetch 后的
提交图、提交归属和工作区状态作出判断。
...
git status --short --branch
git rev-parse HEAD
git rev-parse <远端>/<PR来源>
git merge-base HEAD <远端>/<PR来源>
git log --oneline --left-right HEAD...<远端>/<PR来源>
```

同时记录本任务开始修改前的基线、已确认属于本任务的路径，以及本地独有
提交是否已发布。不得仅凭提交作者是 Codex、bot 或已知协作者判断安全。

#### 6.6.2 判定矩阵

| 状态 | 允许动作 |
| --- | --- |
| 远端仍等于已核验 head | 完成范围与验证门禁后正常提交和推送 |
| 本地 HEAD 是远端 PR head 的祖先，且没有本地独有提交 | 使用 `git merge --ff-only <远端>/<PR来源>`，然后重新检查 diff 和验证 |
| 远端与本地从已记录任务基线分开前进，且本地独有提交全部属于本任务、从未发布 | 可只把这些未发布提交重放到最新远端；发生冲突立即中止重放并停止任务，成功后重新检查范围和验证 |
| 工作区只有已确认的本任务改动，远端线性前进 | 可先按已批准的 commit plan 形成未发布提交，再按上一行处理；不得自动 stash |
| 无法证明祖先/基线关系，存在非本任务本地提交、未识别改动与同步范围重叠、冲突或验证失败 | 停止并报告相关 SHA、路径和原因 |
| 继续操作需要 reset、改写已发布提交或 force-push | 停止 |

“重放未发布提交”只适用于从未出现在远端 PR head 历史中的本任务提交。
它不得用于 rebase 已共享提交，也不得扩大为对共享分支的一般性 rebase。

#### 6.6.3 推送竞态

同步后必须重新执行第 6.5 节的提交和 PR 范围检查，再使用普通 push。普通
push 的服务端 fast-forward 检查是最终竞态保护：

- push 成功：记录实际远端提交 SHA。
- push 因远端再次前进而被拒绝：重新 fetch，并从第 6.6.1 节完整判定一次。
- 第二次仍发生竞态，或重新判定不满足安全条件：停止，避免自动任务持续追逐。
- 不得用 `--force` 或 `--force-with-lease` 绕过此类远端干预。
...
更新已有 PR 时，应显式推送到已核验的原 head 分支，避免本地分支名不同而
意外创建另一条远端分支：

```bash
git push <推送远端> HEAD:<PR来源>
```
...
优先使用仓库现有 PR 模板，并遵循项目使用的语言和标题约定。提交信息的语言要求以第 6.4 节为准。
