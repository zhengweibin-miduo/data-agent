...
from data_agent.ddl_metadata.jobs.job_outbox_store import JobOutboxStore
from data_agent.ddl_metadata.jobs.redis_base_store import RedisBaseStore
from data_agent.ddl_metadata.jobs.redis_job_state_store import RedisJobStateStore
from data_agent.ddl_metadata.jobs.source_lease_store import SourceLeaseStore
...
def question_set_id(questions: list[MetricQuestion]) -> str:
    ...
    Returns:
        与既有协议兼容的 ``sha256:`` 标识。
    """
    return JobCodecStore.question_set_id(questions)
...
class DDLJobStore:
    ...
    def __init__(self, redis: Redis) -> None:
        """绑定已初始化的应用 Redis 客户端。"""
        self._redis = redis
        self._keys = JobKeysStore(self._prefix)
    ...
    async def mark_waiting(
        ...
        return await self.transition(
            ...
            fields={
                ...
                "questions_json": JobCodecStore.questions_json(questions),
    ...
    async def submit_answers(
        ...
        answer_json, answer_hash = JobCodecStore.answers_payload(request.answers)
