...
from data_sync.models import build_desired_tables
...
from ddl_metadata.meta_projection.desired import (
...
from memory.domain.candidates import MemoryVersions, build_accepted_memories
...
class MySQLAcceptedSnapshotPublisher:
    ...
    async def publish(self, snapshot: AcceptedSnapshot) -> None:
        ...
        try:
            ...
            publisher_context = self._generation_locks.write(
                {publisher_lock},
                app_config.data_sync.generation_lock_timeout_seconds,
            )
            async with publisher_context:
                ...
                target_context = self._generation_locks.write(
                    generation_locks,
                    app_config.data_sync.generation_lock_timeout_seconds,
                )
                async with target_context:
                    ...
                    async with MySQLDatabase.session() as session:
                        ...
                        surviving_metrics = (
                            await metadata_repository.existing_object_ids(
