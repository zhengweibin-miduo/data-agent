...
读取根目录 \`AGENTS.md\`、\`code_review.md\`、PR diff 和每条 thread 的代码上下文。确认 \`origin\` 存在、远端分支仍指向 Expected head，并检查工作树是否存在无关修改。如果 head 已变化，立即停止，不要修改、提交或推送。\`gh auth status\` 失败但 Git 推送凭据仍可用时，可以继续修复和推送，只需说明无法自动回复或 resolve thread。
